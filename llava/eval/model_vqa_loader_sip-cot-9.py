...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        demo_1_pos_description = f'this image includes {object_name} and includes other objects, such as ' + ', '.join(demo_1_objects)
        ...
        if is_pdb:
            ...
            demo_image_list[0][0].save(f'pdb_data/d1p_{image_file_name}')
            demo_image_list[0][1].save(f'pdb_data/d1n_{image_file_name}')
            image.save(f'pdb_data/q_{image_file_name}')

        demo_1_objects_wo = list(filter((object_name).__ne__, demo_1_objects))
        if len(demo_1_objects_wo) > 0:
            demo_1_neg_description = f'this image does not include {object_name} but features other things, like ' + ', '.join(demo_1_objects_wo)
        else:
        ...
        for idx in range(self.n_shot):
            ...
            conv.append_message(conv.roles[1], f"Yes, {demo_1_pos_description}.\n")    # ! [pdb] CoT input. - works.
            conv.append_message(conv.roles[0], demo_instruct_list[idx])
            conv.append_message(conv.roles[1], f"No, {demo_1_neg_description}.\n")    # ! [pdb] CoT input. - works.
