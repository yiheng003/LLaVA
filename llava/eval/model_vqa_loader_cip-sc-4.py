...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        demo_1_description = line['p0_description']
        # n0
        demo_2_path = line["n0_image"]
        demo_2_description = line['n0_description']
        # p1
        demo_3_path = line["p1_image"]
        demo_3_description = line['p1_description']
        # n1
        demo_4_path = line["n1_image"]
        demo_4_description = line['n1_description']
        ...
        demo_image_list = [
            ...
            Image.open(os.path.join(self.image_folder, demo_3_path)).convert('RGB'),
            Image.open(os.path.join(self.image_folder, demo_4_path)).convert('RGB'),
        ]
        ...
        conv.append_message(conv.roles[1], f"{demo_1_description} So my answer is yes.\n")
        conv.append_message(conv.roles[0], demo_instruct_list[1])
        conv.append_message(conv.roles[1], f"{demo_2_description} So my answer is no.\n")
        conv.append_message(conv.roles[0], demo_instruct_list[2])
        conv.append_message(conv.roles[1], f"{demo_3_description} So my answer is yes.\n")
        conv.append_message(conv.roles[0], demo_instruct_list[3])
        conv.append_message(conv.roles[1], f"{demo_4_description} So my answer is no.\n")
        # append demonstration image.
        image_list.append(demo_image_list[0])
        image_list.append(demo_image_list[1])
        image_list.append(demo_image_list[2])
        image_list.append(demo_image_list[3])
        ...
        image_size_list.append(demo_image_list[2].size)
        image_size_list.append(demo_image_list[3].size)
        # append question.
        ...
        # image_tensor[3] = -1. * torch.ones_like(image_tensor[0])
